...
app.get('api/employee', (req,res) => {
    ...
    db.query('SELECT first_name, last_name, role_id, manager_id FROM employee', (err, rows) => {
        if(err){
            res.status(500).json({error: err.message});
            return;
        }
...
app.post('api/employee', (req,res) => {
    console.log(`${req.method} was received for /employee`);
    db.query('INSERT INTO employee(first_name, last_name, role_id, manager_id)', (err, rows) => {
        // console.log(rows);
        if(err){
            res.status(500).json({error: err.message});
            return;
...
app.get('api/roles', (req,res) => {
    ...
    db.query('SELECT title, id, department_id FROM employee', (err, rows) => {
        if(err){
            res.status(500).json({error: err.message});
            return;
        }
...
app.post('api/roles', (req,res) => {
    console.log(`${req.method} was received for /roles`);
    db.query('INSERT INTO roles(title, salary, department_id )', (err, rows) => {
        if(err){
            res.status(500).json({error: err.message});
            return;
        }
